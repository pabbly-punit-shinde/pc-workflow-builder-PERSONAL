...
export function Searchbar({ data: navItems = [], sx, ...other }) {
  ...
  // Define modifiedTitle here so it's accessible to both renderItems and renderButton
  const title =
    'Shared - [Add Nuelink Collection Posts from Pinterest Pin ssdsdsdsd rdfdfdfdfdfdfs]';
          ...
          {dataGroups[group].map((item) => {
            ...
            return (
              <Box component="li" key={`${modifiedTitle}${modifiedPath}`} sx={{ display: 'flex' }}>
                <ResultItem path={partsPath} title={partsTitle} groupLabel={searchQuery && group} />
              </Box>
            );
          })}
        </Box>
  ...
  const renderButton = (
    ...
    >
      ...
      Workflow
      <Tooltip title={modifiedTitle} arrow>
        <Label
          sx={{
            ml: 1,
            fontSize: 12,
